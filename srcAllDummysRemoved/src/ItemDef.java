...
public final class ItemDef {
    ...
    public boolean method192(int j) {
        ...
        if (k == -1) return true;
        boolean flag = true;
        if (!Model.method463(k)) flag = false;
        if (l != -1 && !Model.method463(l)) flag = false;
        return flag;
    }
    ...
    public boolean method195(int j) {
        ...
        if (!Model.method463(k)) flag = false;
        if (l != -1 && !Model.method463(l)) flag = false;
        if (i1 != -1 && !Model.method463(i1)) flag = false;
        return flag;
    }
    ...
    public Model method196(int i) {
        ...
        if (i == 0 && aByte205 != 0) model.method475(0, aByte205, 0);
        if (i == 1 && aByte154 != 0) model.method475(0, aByte154, 0);
        if (modifiedModelColors != null) {
            for (int i1 = 0; i1 < modifiedModelColors.length; i1++)
                model.method476(modifiedModelColors[i1], originalModelColors[i1]);
        }
    ...
    public static ItemDef forID(int i) {
        for (int j = 0; j < 10; j++) if (cache[j].id == i) return cache[j];
        ...
        if (itemDef.certTemplateID != -1) itemDef.toNote();
    ...
    private void toNote() {
        ...
        if (c == 'A' || c == 'E' || c == 'I' || c == 'O' || c == 'U') s = "an";
        description =
                ("Swap this note at any bank for " + s + " " + itemDef_1.name + ".").getBytes();
        stackable = true;
    }
    ...
    public static Sprite getSprite(int i, int j, int k) {
        ...
        if (k == -1) k3 = (int) ((double) k3 * 1.5D);
        if (k > 0) k3 = (int) ((double) k3 * 1.04D);
        ...
        model.method482(
                ...
                l3 + model.modelHeight / 2 + itemDef.modelOffset2,
                i4 + itemDef.modelOffset2);
        ...
        if (k > 0) {
            ...
            for (int k5 = 31; k5 >= 0; k5--) {
                for (int l4 = 31; l4 >= 0; l4--)
                    if (sprite2.myPixels[k5 + l4 * 32] == 0
                            && k5 > 0
                            && l4 > 0
        ...
        if (itemDef.stackable) sprite2.anInt1444 = 33;
        else sprite2.anInt1444 = 32;
        sprite2.anInt1445 = j;
        return sprite2;
    }
    ...
    public Model method201(int i) {
        if (stackIDs != null && i > 1) {
            int j = -1;
            for (int k = 0; k < 10; k++)
                if (i >= stackAmounts[k] && stackAmounts[k] != 0) j = stackIDs[k];

            if (j != -1) return forID(j).method201(1);
        }
    ...
    public Model method202(int i) {
        if (stackIDs != null && i > 1) {
            ...
            if (j != -1) return forID(j).method202(1);
        }
    ...
    private void readValues(Stream stream) {
        do {
            ...
            if (i == 1) modelID = stream.readUnsignedWord();
            ...
            else if (i == 5) modelRotation1 = stream.readUnsignedWord();
            else if (i == 6) modelRotation2 = stream.readUnsignedWord();
            else if (i == 7) {
                ...
                if (groundActions[i - 30].equalsIgnoreCase("hidden")) groundActions[i - 30] = null;
            } else if (i >= 35 && i < 40) {
            ...
            else if (i == 113) anInt196 = stream.readSignedByte();
