...
public final class VarBit {
    ...
    public static void unpackConfig(StreamLoader streamLoader) {
        ...
        if (cache == null) cache = new VarBit[cacheSize];
        for (int j = 0; j < cacheSize; j++) {
            if (cache[j] == null) cache[j] = new VarBit();
    ...
    private void readValues(Stream stream) {
        do {
            ...
            else if (j == 2) aBoolean651 = true;
            else if (j == 3) stream.readDWord();
            else if (j == 4) stream.readDWord();
