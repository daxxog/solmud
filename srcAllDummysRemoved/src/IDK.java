...
public final class IDK {
    ...
    public static void unpackConfig(StreamLoader streamLoader) {
        ...
        if (cache == null) cache = new IDK[length];
        for (int j = 0; j < length; j++) {
            if (cache[j] == null) cache[j] = new IDK();
    ...
    private void readValues(Stream stream) {
        do {
            ...
            if (i == 0) return;
            if (i == 1) anInt657 = stream.readUnsignedByte();
            else if (i == 2) {
                int j = stream.readUnsignedByte();
                anIntArray658 = new int[j];
                for (int k = 0; k < j; k++) anIntArray658[k] = stream.readUnsignedWord();

            } else if (i == 3) aBoolean662 = true;
            else if (i >= 40 && i < 50) anIntArray659[i - 40] = stream.readUnsignedWord();
            else if (i >= 50 && i < 60) anIntArray660[i - 50] = stream.readUnsignedWord();
            else if (i >= 60 && i < 70) anIntArray661[i - 60] = stream.readUnsignedWord();
            else System.out.println("Error unrecognised config code: " + i);
        } while (true);
    }

    public boolean method537() {
        if (anIntArray658 == null) return true;
        boolean flag = true;
        for (int j = 0; j < anIntArray658.length; j++)
            if (!Model.method463(anIntArray658[j])) flag = false;

        return flag;
    }

    public Model method538() {
        if (anIntArray658 == null) return null;
    ...
    public boolean method539() {
        boolean flag1 = true;
        for (int i = 0; i < 5; i++)
            if (anIntArray661[i] != -1 && !Model.method463(anIntArray661[i])) flag1 = false;
