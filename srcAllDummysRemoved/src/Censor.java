...
final class Censor {
    ...
    private static void readValues(
            Stream stream, Stream stream_1, Stream stream_2, Stream stream_3) {
        readBadEnc(stream_1);
    ...
    private static void readFragmentsEnc(Stream stream) {
        ...
        for (int i = 0; i < anIntArray620.length; i++) anIntArray620[i] = stream.readUnsignedWord();
    }
    ...
    private static void method495(char ac[]) {
        int i = 0;
        for (int j = 0; j < ac.length; j++) {
            if (method496(ac[j])) ac[i] = ac[j];
            else ac[i] = ' ';
            if (i == 0 || ac[i] != ' ' || ac[i - 1] != ' ') i++;
        }
        for (int k = i; k < ac.length; k++) ac[k] = ' ';
    }
    ...
    private static void method498(char ac[], char ac1[]) {
        for (int j = 0; j < ac.length; j++)
            if (ac1[j] != '*' && isUpperCaseLetter(ac[j])) ac1[j] = ac[j];
    }
    ...
    private static void method499(char ac[]) {
        ...
        for (int j = 0; j < ac.length; j++) {
            ...
            if (isLetter(c)) {
                if (flag) {
                    if (isLowerCaseLetter(c)) flag = false;
                } else if (isUpperCaseLetter(c)) ac[j] = (char) ((c + 97) - 65);
            } else {
                flag = true;
            }
        }
    ...
    private static void method502(char ac[], char ac1[], char ac2[], char ac3[]) {
        ...
        for (int k = 0; k <= ac.length - ac1.length; k += j) {
            ...
            while (l < ac.length) {
                ...
                if (i1 >= ac1.length || !method517(c)) break;
                l++;
            }
            if (i1 >= ac1.length) {
                boolean flag1 = false;
    ...
    private static int method503(char ac[], char ac1[], int j) {
        if (j == 0) return 2;
        for (int k = j - 1; k >= 0; k--) {
            if (!method517(ac[k])) break;
            if (ac[k] == '@') return 3;
        }

        int l = 0;
        for (int i1 = j - 1; i1 >= 0; i1--) {
            if (!method517(ac1[i1])) break;
            if (ac1[i1] == '*') l++;
        }
    ...
    private static int method504(char ac[], int i, char ac1[]) {
        ...
        for (int j = i + 1; j < ac1.length; j++) {
            if (!method517(ac1[j])) break;
            if (ac1[j] == '.' || ac1[j] == ',') return 3;
        }
        int k = 0;
        for (int l = i + 1; l < ac1.length; l++) {
            if (!method517(ac[l])) break;
            if (ac[l] == '*') k++;
        }
    ...
    private static void method506(char ac[], char ac1[], int i, char ac2[], char ac3[]) {
        if (ac1.length > ac3.length) return;
        int j;
    ...
    private static void method509(byte abyte0[][], char ac[], char ac1[]) {
        ...
        for (int k = 0; k <= ac.length - ac1.length; k += j) {
            ...
            while (l < ac.length && (!flag2 || !flag3)) {
                ...
                char c = ac[l];
                char c2 = '\0';
                if (l + 1 < ac.length) c2 = ac[l + 1];
                if (i1 < ac1.length && (k1 = method512(c2, c, ac1[i1])) > 0) {
                    if (k1 == 1 && isDigit(c)) flag2 = true;
                    if (k1 == 2 && (isDigit(c) || isDigit(c2))) flag2 = true;
                    l += k1;
                    i1++;
                    continue;
                }
                ...
                if (i1 >= ac1.length || !method518(c)) break;
                if (method517(c) && c != '\'') flag1 = true;
                if (isDigit(c)) flag3 = true;
                l++;
            ...
            if (i1 >= ac1.length && (!flag2 || !flag3)) {
                ...
                if (!flag1) {
                    ...
                    if (k - 1 >= 0) c1 = ac[k - 1];
                    char c3 = ' ';
                    if (l < ac.length) c3 = ac[l];
                    byte byte0 = method513(c1);
                    byte byte1 = method513(c3);
                    if (abyte0 != null && method510(byte0, abyte0, byte1)) flag4 = false;
                } else {
                    boolean flag5 = false;
                    boolean flag6 = false;
                    if (k - 1 < 0 || method517(ac[k - 1]) && ac[k - 1] != '\'') flag5 = true;
                    if (l >= ac.length || method517(ac[l]) && ac[l] != '\'') flag6 = true;
                    if (!flag5 || !flag6) {
                        boolean flag7 = false;
                        int k2 = k - 2;
                        if (flag5) k2 = k;
                        for (; !flag7 && k2 < l; k2++)
                            if (k2 >= 0 && (!method517(ac[k2]) || ac[k2] == '\'')) {
                                ...
                                for (j3 = 0; j3 < 3; j3++) {
                                    if (k2 + j3 >= ac.length
                                            || method517(ac[k2 + j3]) && ac[k2 + j3] != '\'') break;
                                    ac2[j3] = ac[k2 + j3];
                                }
                                ...
                                if (j3 < 3
                                    ...
                                    flag8 = false;
                                if (flag8 && !method523(ac2)) flag7 = true;
                            }
    ...
    private static boolean method510(byte byte0, byte abyte0[][], byte byte2) {
        int i = 0;
        if (abyte0[i][0] == byte0 && abyte0[i][1] == byte2) return true;
        int j = abyte0.length - 1;
        if (abyte0[j][0] == byte0 && abyte0[j][1] == byte2) return true;
        do {
            int k = (i + j) / 2;
            if (abyte0[k][0] == byte0 && abyte0[k][1] == byte2) return true;
            if (byte0 < abyte0[k][0] || byte0 == abyte0[k][0] && byte2 < abyte0[k][1]) j = k;
            else i = k;
        } while (i != j && i + 1 != j);
        return false;
    }
    ...
    private static int method511(char c, char c1, char c2) {
        if (c1 == c) return 1;
        if (c1 == 'o' && c == '0') return 1;
        if (c1 == 'o' && c == '(' && c2 == ')') return 2;
        if (c1 == 'c' && (c == '(' || c == '<' || c == '[')) return 1;
        if (c1 == 'e' && c == '\u20AC') return 1;
        if (c1 == 's' && c == '$') return 1;
        return c1 != 'l' || c != 'i' ? 0 : 1;
    }
    ...
    private static int method512(char c, char c1, char c2) {
        ...
        if (c2 >= 'a' && c2 <= 'm') {
            if (c2 == 'a') {
                if (c1 == '4' || c1 == '@' || c1 == '^') return 1;
                return c1 != '/' || c != '\\' ? 0 : 2;
            }
            if (c2 == 'b') {
                if (c1 == '6' || c1 == '8') return 1;
                return (c1 != '1' || c != '3') && (c1 != 'i' || c != '3') ? 0 : 2;
            }
            if (c2 == 'c') return c1 != '(' && c1 != '<' && c1 != '{' && c1 != '[' ? 0 : 1;
            if (c2 == 'd') return (c1 != '[' || c != ')') && (c1 != 'i' || c != ')') ? 0 : 2;
            if (c2 == 'e') return c1 != '3' && c1 != '\u20AC' ? 0 : 1;
            if (c2 == 'f') {
                if (c1 == 'p' && c == 'h') return 2;
                return c1 != '\243' ? 0 : 1;
            }
            if (c2 == 'g') return c1 != '9' && c1 != '6' && c1 != 'q' ? 0 : 1;
            if (c2 == 'h') return c1 != '#' ? 0 : 1;
            if (c2 == 'i')
                return c1 != 'y' && c1 != 'l' && c1 != 'j' && c1 != '1' && c1 != '!' && c1 != ':'
                                && c1 != ';' && c1 != '|'
                        ? 0
                        : 1;
            if (c2 == 'j') return 0;
            if (c2 == 'k') return 0;
            if (c2 == 'l') return c1 != '1' && c1 != '|' && c1 != 'i' ? 0 : 1;
            if (c2 == 'm') return 0;
        }
        if (c2 >= 'n' && c2 <= 'z') {
            if (c2 == 'n') return 0;
            ...
            if (c2 == 'q') return 0;
            if (c2 == 'r') return 0;
            if (c2 == 's') return c1 != '5' && c1 != 'z' && c1 != '$' && c1 != '2' ? 0 : 1;
            if (c2 == 't') return c1 != '7' && c1 != '+' ? 0 : 1;
            if (c2 == 'u') {
                if (c1 == 'v') return 1;
                return (c1 != '\\' || c != '/')
                                && (c1 != '\\' || c != '|')
        ...
        if (c2 >= '0' && c2 <= '9') {
            ...
            if (c2 == '1') return c1 != 'l' ? 0 : 1;
            else return 0;
        }
        if (c2 == ',') return c1 != '.' ? 0 : 1;
        if (c2 == '.') return c1 != ',' ? 0 : 1;
        if (c2 == '!') return c1 != 'i' ? 0 : 1;
        else return 0;
    }
    ...
    private static byte method513(char c) {
        ...
        if (c == '\'') return 28;
        if (c >= '0' && c <= '9') return (byte) ((c - 48) + 29);
        else return 27;
    }
    ...
    private static void method514(char ac[]) {
        ...
        while ((j = method515(ac, k)) != -1) {
            ...
            for (int j1 = k; j1 >= 0 && j1 < j && !flag; j1++)
                if (!method517(ac[j1]) && !method518(ac[j1])) flag = true;

            if (flag) l = 0;
            if (l == 0) i1 = j;
            k = method516(ac, j);
            int k1 = 0;
            for (int l1 = j; l1 < k; l1++) k1 = (k1 * 10 + ac[l1]) - 48;

            if (k1 > 255 || k - j > 8) l = 0;
            else l++;
            if (l == 4) {
                for (int i2 = i1; i2 < k; i2++) ac[i2] = '*';
    ...
    private static int method515(char ac[], int i) {
        for (int k = i; k < ac.length && k >= 0; k++) if (ac[k] >= '0' && ac[k] <= '9') return k;

        return -1;
    }

    private static int method516(char ac[], int j) {
        for (int k = j; k < ac.length && k >= 0; k++) if (ac[k] < '0' || ac[k] > '9') return k;
        return ac.length;
    }
    ...
    private static boolean method523(char ac[]) {
        boolean flag = true;
        for (int i = 0; i < ac.length; i++) if (!isDigit(ac[i]) && ac[i] != 0) flag = false;

        if (flag) return true;
        int j = method524(ac);
        int k = 0;
        int l = anIntArray620.length - 1;
        if (j == anIntArray620[k] || j == anIntArray620[l]) return true;
        do {
            int i1 = (k + l) / 2;
            if (j == anIntArray620[i1]) return true;
            if (j < anIntArray620[i1]) l = i1;
            else k = i1;
        } while (k != l && k + 1 != l);
        return false;
    }
    ...
    private static int method524(char ac[]) {
        ...
        for (int l = 0; l < ac.length; l++) {
            ...
            if (c >= 'a' && c <= 'z') k = k * 38 + ((c - 97) + 1);
            else if (c == '\'') k = k * 38 + 27;
            else if (c >= '0' && c <= '9') k = k * 38 + ((c - 48) + 28);
