...
final class Class39 {
    ...
    private float method541(int i, int j, float f) {
        float f1 =
                (float) anIntArrayArrayArray667[i][0][j]
                        + f
                                * (float)
                                        (anIntArrayArrayArray667[i][1][j]
                                                - anIntArrayArrayArray667[i][0][j]);
        f1 *= 0.001525879F;
    ...
    private float method543(float f, int i, int j) {
        float f1 =
                (float) anIntArrayArrayArray666[j][0][i]
                        + f
                                * (float)
                                        (anIntArrayArrayArray666[j][1][i]
                                                - anIntArrayArrayArray666[j][0][i]);
        f1 *= 0.0001220703F;
        return method542(f1);
    }
    ...
    public int method544(int i, float f) {
        ...
        for (int k = 1; k < anIntArray665[i]; k++) {
            ...
            aFloatArrayArray669[i][k * 2] =
                    aFloatArrayArray669[i][k * 2 - 1] * f4 + aFloatArrayArray669[i][k * 2 - 2] * f5;
            for (int j1 = k * 2 - 1; j1 >= 2; j1--)
                aFloatArrayArray669[i][j1] +=
                        aFloatArrayArray669[i][j1 - 1] * f4 + aFloatArrayArray669[i][j1 - 2] * f5;
        ...
        if (i == 0) {
            for (int l = 0; l < anIntArray665[0] * 2; l++) aFloatArrayArray669[0][l] *= aFloat671;
        }
    ...
    public void method545(Stream stream, Class29 class29) {
        ...
        if (i != 0) {
            ...
            if (j != 0 || anIntArray668[1] != anIntArray668[0]) class29.method326(stream);
        } else {
