...
public final class ISAACRandomGen {
    ...
    private void isaac() {
        ...
        for (int i = 0; i < 256; i++) {
            int j = memory[i];
            if ((i & 3) == 0) accumulator ^= accumulator << 13;
            else if ((i & 3) == 1) accumulator ^= accumulator >>> 6;
            else if ((i & 3) == 2) accumulator ^= accumulator << 2;
            else if ((i & 3) == 3) accumulator ^= accumulator >>> 16;
