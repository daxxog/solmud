...
public final class OnDemandFetcher extends OnDemandFetcherParent implements Runnable {
    ...
    private boolean crcMatches(int i, int j, byte abyte0[]) {
        if (abyte0 == null || abyte0.length < 2) return false;
    ...
    public void start(StreamLoader streamLoader, client client1) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int l = 0; l < j; l++) versions[i][l] = stream.readUnsignedWord();
        }
        ...
        for (int k = 0; k < 4; k++) {
            ...
            crcs[k] = new int[i1];
            for (int l1 = 0; l1 < i1; l1++) crcs[k][l1] = stream_1.readDWord();
        }
        ...
        for (int j2 = 0; j2 < j1; j2++) anIntArray1360[j2] = stream2.readUnsignedWord();
    ...
    private void closeRequest(OnDemandData onDemandData) {
        try {
            if (socket == null) {
                ...
                for (int j = 0; j < 8; j++) inputStream.read();
            ...
            if (onDemandData.incomplete) ioBuffer[3] = 2;
            else if (!clientInstance.loggedIn) ioBuffer[3] = 1;
            else ioBuffer[3] = 0;
    ...
    public void method560(int i, int j) {
        if (clientInstance.decompressors[0] == null) return;
        if (versions[j][i] == 0) return;
        if (fileStatus[j][i] == 0) return;
        if (anInt1332 == 0) return;
    ...
    public int method562(int i, int k, int l) {
        ...
        for (int j1 = 0; j1 < mapIndices1.length; j1++)
            if (mapIndices1[j1] == i1)
                if (i == 0) return mapIndices2[j1];
                else return mapIndices3[j1];
        return -1;
    }
    ...
    public void method563(byte byte0, int i, int j) {
        ...
        if (crcMatches(versions[i][j], crcs[i][j], abyte0)) return;
        fileStatus[i][j] = byte0;
        if (byte0 > anInt1332) anInt1332 = byte0;
        totalFiles++;
    }

    public boolean method564(int i) {
        for (int k = 0; k < mapIndices1.length; k++) if (mapIndices3[k] == i) return true;
        return false;
    }
    ...
    private void handleFailed() {
        ...
        for (OnDemandData onDemandData = (OnDemandData) requested.reverseGetFirst();
                onDemandData != null;
                onDemandData = (OnDemandData) requested.reverseGetNext())
            if (onDemandData.incomplete) uncompletedCount++;
            else completedCount++;

        while (uncompletedCount < 10) {
            OnDemandData onDemandData_1 = (OnDemandData) aClass19_1368.popHead();
            if (onDemandData_1 == null) break;
            if (fileStatus[onDemandData_1.dataType][onDemandData_1.ID] != 0) filesLoaded++;
    ...
    private void checkReceived() {
        ...
        while (onDemandData != null) {
            ...
            if (clientInstance.decompressors[0] != null)
                abyte0 =
                        clientInstance.decompressors[onDemandData.dataType + 1].decompress(
                                onDemandData.ID);
            if (!crcMatches(
        ...
        while (uncompletedCount == 0 && completedCount < 10) {
            ...
            while (onDemandData != null) {
                if (fileStatus[onDemandData.dataType][onDemandData.ID] != 0) {
                    ...
                    statusString =
                            "Loading extra files - " + (filesLoaded * 100) / totalFiles + "%";
                    completedCount++;
