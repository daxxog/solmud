...
public class RSApplet extends Applet
    ...
    public void run() {
        ...
        if (gameFrame != null) gameFrame.addWindowListener(this);
        ...
        int i1 = 0;
        int j1 = 0;
        for (int k1 = 0; k1 < 10; k1++) aLongArray7[k1] = System.currentTimeMillis();
        ...
        while (anInt4 >= 0) {
            ...
            if (k > delayTime) k = delayTime;
            aLongArray7[i] = l1;
            i = (i + 1) % 10;
            if (k > 1) {
                for (int k2 = 0; k2 < 10; k2++) if (aLongArray7[k2] != 0L) aLongArray7[k2] += k;
            }
            if (k < minDelay) k = minDelay;
            try {
                Thread.sleep(k);
            ...
            i1 &= 0xff;
            if (delayTime > 0) fps = (1000 * j) / (delayTime * 256);
            processDrawing();
            if (shouldDebug) {
    ...
    public final void focusLost(FocusEvent focusevent) {
        awtFocus = false;
        for (int i = 0; i < 128; i++) keyArray[i] = 0;
    }
    ...
    Component getGameComponent() {
        if (gameFrame != null) return gameFrame;
        else return this;
    }
