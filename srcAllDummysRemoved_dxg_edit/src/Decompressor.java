...
final class Decompressor {
    ...
    public synchronized byte[] decompress(int i) {
        try {
            ...
            // if(i1 < 0 || i1 > 0x7a120)
            //	return null;
            if (j1 <= 0 || (long) j1 > dataFile.length() / 520L) return null;
            ...
            for (int l1 = 0; k1 < i1; l1++) {
                ...
                int i3 =
                        ((buffer[4] & 0xff) << 16) + ((buffer[5] & 0xff) << 8) + (buffer[6] & 0xff);
                int j3 = buffer[7] & 0xff;
                if (k2 != i || l2 != l1 || j3 != anInt311) return null;
                if (i3 < 0 || (long) i3 > dataFile.length() / 520L) return null;
                for (int k3 = 0; k3 < i2; k3++) abyte0[k1++] = buffer[k3 + 8];
    ...
    public synchronized boolean method234(int i, byte abyte0[], int j) {
        ...
        if (!flag) flag = method235(false, j, i, abyte0);
        return flag;
    }
    ...
    private synchronized boolean method235(boolean flag, int j, int k, byte abyte0[]) {
        try {
            ...
            if (flag) {
                ...
                if (l <= 0 || (long) l > dataFile.length() / 520L) return false;
            } else {
            ...
            for (int l1 = 0; j1 < k; l1++) {
                ...
                if (flag) {
                    ...
                    if (j2 == 8) {
                        ...
                        if (i3 != j || j3 != l1 || k3 != anInt311) return false;
                        if (i2 < 0 || (long) i2 > dataFile.length() / 520L) return false;
                    }
                }
                if (i2 == 0) {
                    flag = false;
